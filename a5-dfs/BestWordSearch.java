...
/**
 * Word search game based on WordSearchGame.java provided
 *
...
public class BestWordSearch implements WordSearchGame {
   ...
   */
   public BestWordSearch() {
    ...
    /**
    * Loads the lexicon into a d.s.
    * 
    * @param fileName A string containing the name of the file to be opened.
    * @throws IllegalArgumentException if fileName is null
    * @throws IllegalArgumentException if fileName can't be opened.
    */
   public void loadLexicon(String fileName) {
      lexicon = new TreeSet<String>(); 
    //Checks if file isn't null
      if (fileName == null) {
   ...
   /**
    * Stores the incoming array in a d.s that will make
    * it easier to find the words
    * 
    * @param letterArray array that stores the contents of the
   ...
   /**
    * Creates a readable representation of the board, suitable for printing to
    *   stdout
    */
   ...
   public SortedSet<String> getAllValidWords(int minimumWordLength) {
      ...
      for (int i = 0; i < height; i++) {
         for (int j = 0; j < width; j ++) {
            wordsBuilt = board[i][j];
            if (isValidWord(wordsBuilt) && wordsBuilt.length() >= minimumWordLength) {
               allWords.add(wordsBuilt);
            }
            if (isValidPrefix(wordsBuilt)) {
   ...
   private void dfs2(int x, int y, int min) {
      ...
      for (Position p : start.neighbors()) {
         if (!isVisited(p)) {
            visit(p);
            if (isValidPrefix(wordsBuilt + board[p.x][p.y])) {
               wordsBuilt += board[p.x][p.y];
               path2.add(p);
               if (isValidWord(wordsBuilt) && wordsBuilt.length() >= min) {
                  allWords.add(wordsBuilt);
               }
               dfs2(p.x, p.y, min);
               //Backtracking
               path2.remove(p);
               int endIndex = wordsBuilt.length() - board[p.x][p.y].length();
               wordsBuilt = wordsBuilt.substring(0, endIndex);
            }
         }
      }
      markAllUnvisited(); 
      ...
      for (int i = 0; i < height; i++) {
         for (int j = 0; j < width; j ++) {
            ...
            if (wordToCheck.startsWith(board[i][j])) {
               ...
               if (!wordToCheck.equals(wordsBuilt)) {
                  path2.remove(pos);
               }
               else {
   ...
   */
   private void dfs(int x, int y, String wordToCheck) {
      ...
      for (Position p: start.neighbors()) {
         if (!isVisited(p)) {
            visit(p);
            if (wordToCheck.startsWith(wordsBuilt + board[p.x][p.y])) {
               wordsBuilt += board[p.x][p.y]; 
               path2.add(p);
